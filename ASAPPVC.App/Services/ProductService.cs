...
namespace ASAPPVC.App.Services
{
 ...
 Task<Result<List<ProductListVm>>> GetListByIdsOrCodesAsync(IEnumerable<Guid>? ids = null, IEnumerable<string>? codes = null, CancellationToken ct = default);
 ...
 /// <summary>
 /// Retrieves all active components mapped to their ProductComponent VM for use in product forms.
 /// Returns a list suitable for direct assignment to <see cref="ProductFormVm.AvailableProductComponents"/>.
 /// </summary>
 Task<Result<List<ProductComponentVm>>> GetAvailableComponentsAsync(CancellationToken ct = default);

 /// <summary>
 /// Retrieves a single product mapped to the form view model used for create/edit screens.
 /// Useful when prefilling an edit form or preparing a blank create form.
 /// </summary>
 Task<Result<ProductFormVm>> GetFormAsync(Guid? id = null, string? code = null, CancellationToken ct = default);
 }
 ...
 //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\\
 // Retrieves all active components as a list for product form consumption
 public async Task<Result<List<ProductComponentVm>>> GetAvailableComponentsAsync(CancellationToken ct = default)
 {
 try
 {
 ...
 .Select(c => new ProductComponentVm
 {
 ...
 return Result<List<ProductComponentVm>>.Success(list);
 }
 catch (Exception ex)
 {
 return Result<List<ProductComponentVm>>.Fail($"Failed to build component list: {ex.Message}");
 }
 }

 //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\\
 // Retrieves a single product mapped to the form VM (for create/edit screens)
 public async Task<Result<ProductFormVm>> GetFormAsync(Guid? id = null, string? code = null, CancellationToken ct = default)
 {
 try
 {
 ...
 {
 var empty = new ProductFormVm();
 return Result<ProductFormVm>.Success(empty);
 }

 var product = await _products.GetByIdOrCodeWithComponentsAsync(id, code, asNoTracking: true, ct: ct);
 if (product is null)
 return Result<ProductFormVm>.Fail("Product not found.");

 var formVm = _mapper.ToFormVm(product);
 return Result<ProductFormVm>.Success(formVm);
 }
 catch (Exception ex)
 {
 return Result<ProductFormVm>.Fail($"Failed to retrieve product form: {ex.Message}");
 }
 }
 ...
 public async Task<Result<ProductDetailVm>> GetDetailAsync(Guid? id = null, string? code = null, CancellationToken ct = default)
 {
 try
 {
 ...
 public async Task<Result<Product>> GetDomainAsync(Guid? id = null, string? code = null, CancellationToken ct = default)
 {
 try
 {
 ...
 public async Task<Result<List<ProductListVm>>> GetListByIdsOrCodesAsync(IEnumerable<Guid>? ids = null, IEnumerable<string>? codes = null, CancellationToken ct = default)
 {
 try
 {
 ...
 public async Task<Result<bool>> ExistsAsync(string code, Guid? excludeId = null, CancellationToken ct = default)
 {
