...
namespace ASAPPVC.App.Services
{
    ...
    public interface IProductService
    {
        ...
        Task<Result<List<ProductListVm>>> GetAvailableProductsAsync(CancellationToken ct = default);
    ...
    IProductMapper productMapper) : IProductService
    {
        ...
        //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\\
        // Retrieves available (active) products only
        public async Task<Result<List<ProductListVm>>> GetAvailableProductsAsync(CancellationToken ct = default)
        {
            try
            {
                var products = await _products.GetListAsync(asNoTracking: true, ct: ct);
                var available = products.Where(p => p.IsActive).ToList();
                var listVms = available.Select(p => _mapper.ToListVm(p)).ToList();
                return Result<List<ProductListVm>>.Success(listVms);
            }
            catch (Exception ex)
            {
                return Result<List<ProductListVm>>.Fail($"Failed to retrieve available products: {ex.Message}");
            }
        }
